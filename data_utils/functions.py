...
from keras.layers import Input, Dense, Flatten, Convolution1D, Merge, Activation, AtrousConvolution1D
...
def generate_input(f_set, ph_set, t, mode='simple'):
    Amax = 1
    wave_ = []

    for s in range(len(mode)):
        if mode[s] == 'simple':
            sine = Amax * np.sin(2 * np.pi * f_set[s] * t)

        elif mode[s] == 'mixture1':
            # Mixture of 2 sinusoids at different frequencies
            sine1 = Amax * np.sin(2 * np.pi * f_set[s] * t)
            sine2 = Amax * np.sin(2 * np.pi * 2*f_set[s] * t)
            sine = sine1 + sine2

        elif mode[s] == 'mixture2':
            # Mixture of 3 sinusoids at different frequencies
            sine1 = Amax * np.sin(2 * np.pi * f_set[s] * t)
            sine2 = Amax * np.sin(2 * np.pi * 110 * t)
            sine3 = Amax * np.sin(2 * np.pi * 60 * t)
            sine = sine1 + sine2 + sine3

        elif mode[s] == 'mixture3':
            # Mixture of sinusoids at different frequencies with random phases
            sine1 = Amax * np.sin(2 * np.pi * f_set[0] * t + ph_set[0])
            sine2 = Amax * np.sin(2 * np.pi * f_set[0]*2 * t + ph_set[1])
            sine3 = Amax * np.sin(2 * np.pi * f_set[0]/2 * t + ph_set[2])
            sine = sine1 + sine2 + sine3
...
def complex_model(data_, target_, nb_filters_, framerate_, loading='False', path=""):
    ...
    for _ in range(int(np.log2(np.floor(data_.shape[1])))):
